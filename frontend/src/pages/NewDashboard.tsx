...
// ── Warm colour palette (no blue/purple) ──────────────────────────────────
const WARM = {
  orange:    '#f97316',
  amber:     '#f59e0b',
  red:       '#ef4444',
...
// ── Helpers ────────────────────────────────────────────────────────────────
function StatCard({ label, value, sub, icon: Icon, iconBg, iconColor, badgeColor }: {
  label: string; value: string; sub: string; icon: any
  iconBg: string; iconColor: string; badgeColor?: string
}) {
  return (
    <div style={{ background: WARM.card, border: `1px solid ${WARM.border}`, borderRadius: '10px', padding: '14px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
        <span style={{ fontSize: '11px', fontWeight: 600, color: WARM.muted }}>{label}</span>
        <div style={{ padding: '5px', background: iconBg, borderRadius: '6px' }}>
          <Icon size={13} color={iconColor} />
        </div>
      </div>
      <div style={{ fontSize: '22px', fontWeight: 900, color: badgeColor ?? WARM.text, marginBottom: '5px', lineHeight: 1 }}>{value}</div>
...
export function NewDashboard() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      const delayed = projects.filter(p => p.snapshots?.[p.snapshots.length - 1]?.is_delayed).length
      ...
      const topPreds = [...alertsData].sort((a: any, b: any) => (+b.risk_score||0)-(+a.risk_score||0)).slice(0,5)
      if (topPreds.length) setTopRisk(topPreds.map((a: any) => ({ name: a.project?.project_name ?? 'Unknown', ministry: a.project?.ministry?.name ?? 'N/A', score: Math.round(+(a.risk_score??0)*100) })))

      const sorted = [...alertsData].sort((a: any, b: any) => new Date(b.predicted_at).getTime() - new Date(a.predicted_at).getTime()).slice(0,5)
      if (sorted.length) setAlerts(sorted.map((a: any) => {
  ...
  return (
    <DashboardLayout searchValue={search} onSearchChange={setSearch}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: '10px', marginBottom: '14px' }}>
        <StatCard label="Total Projects"         value={stats.totalProjects.toLocaleString()}
          sub={`▲ ${stats.totalProjectsGrowth}% · 17 Ministries`}
          icon={Building2} iconBg="#fef3c7" iconColor={WARM.amber} />
        <StatCard label="Original Cost"          value={`₹${(stats.originalCost/100).toFixed(0)}K Cr`}
          sub="Sanctioned value"
          icon={DollarSign} iconBg="#f0fdf4" iconColor="#22c55e" />
        <StatCard label="Revised Cost"           value={`₹${(stats.revisedCost/100).toFixed(0)}K Cr`}
          sub={`▲ ${stats.revisedCostGrowth}% escalation`}
          icon={Zap} iconBg="#fff7ed" iconColor={WARM.orange} />
        <StatCard label="Cumulative Exp."        value={`₹${(stats.cumulativeExpenditure/100).toFixed(0)}K Cr`}
          sub={`${stats.expenditurePercent}% of revised`}
          icon={BarChart3} iconBg="#fdf4ff" iconColor="#a855f7" />
        <StatCard label="High Risk Projects"     value={String(stats.highRiskProjects)}
          sub={`▲ ${stats.highRiskGrowth}% increase`}
          icon={AlertTriangle} iconBg="#fee2e2" iconColor={WARM.red} badgeColor={WARM.red} />
        <StatCard label="Projects Delayed"       value={String(stats.projectsDelayed)}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.1fr 0.9fr 0.9fr', gap: '12px', marginBottom: '12px' }}>
        ...
        {/* India map */}
        <div style={{ background: WARM.card, border: `1px solid ${WARM.border}`, borderRadius: '10px', overflow: 'hidden' }}>
          <StateDistributionMap />
        </div>

        {/* Risk donut */}
        <div style={{ background: WARM.card, border: `1px solid ${WARM.border}`, borderRadius: '10px', padding: '16px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
            {risk.map((r, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '7px 10px', background: '#f8fafc', borderRadius: '6px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '7px' }}>
                  <div style={{ width: '9px', height: '9px', background: r.color, borderRadius: '2px' }} />
            ...
            {sectors.map((s, i) => (
              <div key={i}>
                ...
                <div style={{ height: '6px', background: '#f1f5f9', borderRadius: '3px' }}>
                  <div style={{ width: `${(s.count / maxSector) * 100}%`, height: '100%', background: s.color, borderRadius: '3px', transition: 'width .4s' }} />
                </div>
              </div>
            ))}
          </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 0.8fr', gap: '12px', marginBottom: '12px' }}>
        ...
        <div style={{ background: WARM.card, border: `1px solid ${WARM.border}`, borderRadius: '10px', padding: '16px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
            ...
            <span style={{ fontSize: '11px', color: WARM.muted }}>Last 5 Years</span>
          </div>
          <svg viewBox="0 0 500 160" style={{ width: '100%', overflow: 'visible' }}>
            ...
            {/* Cost overrun (orange) */}
            <polyline points="40,138 130,126 220,117 310,106 400,94 490,82" fill="none" stroke={WARM.orange} strokeWidth={2.5} strokeLinejoin="round" />
            {[138,126,117,106,94,82].map((y,i)=><circle key={i} cx={40+i*90} cy={y} r={3.5} fill={WARM.orange} />)}
            <text x={494} y={82} fontSize={9} fontWeight={800} fill={WARM.orange}>32%</text>
            {/* Time overrun (amber) */}
            <polyline points="40,145 130,140 220,133 310,125 400,116 490,111" fill="none" stroke={WARM.amber} strokeWidth={2.5} strokeLinejoin="round" strokeDasharray="5 3" />
            {[145,140,133,125,116,111].map((y,i)=><circle key={i} cx={40+i*90} cy={y} r={3.5} fill={WARM.amber} />)}
          ...
          <div style={{ display: 'flex', gap: '20px', justifyContent: 'center', marginTop: '8px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '11.5px', color: WARM.muted }}>
              <div style={{ width: '20px', height: '3px', background: WARM.orange, borderRadius: '2px' }} /> Cost Overrun (%)
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '11.5px', color: WARM.muted }}>
              <div style={{ width: '20px', height: '2px', background: WARM.amber, borderRadius: '2px', borderBottom: '2px dashed ' + WARM.amber }} /> Time Overrun (%)
            </div>
          </div>
        </div>
        ...
        {/* Top 5 risk */}
        <div style={{ background: WARM.card, border: `1px solid ${WARM.border}`, borderRadius: '10px', padding: '16px' }}>
          <div style={{ fontSize: '13px', fontWeight: 800, color: WARM.text, marginBottom: '12px' }}>Top 5 High Risk Projects</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {topRisk.map((p, i) => (
              <div key={i} style={{ padding: '10px 12px', background: '#fafafa', borderRadius: '7px', borderLeft: `3px solid ${p.score >= 90 ? WARM.red : p.score >= 85 ? WARM.orange : WARM.amber}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                ...
                <div style={{ padding: '3px 9px', background: p.score >= 90 ? WARM.red : p.score >= 85 ? WARM.orange : WARM.amber, color: 'white', borderRadius: '6px', fontSize: '13px', fontWeight: 900 }}>{p.score}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{ background: WARM.card, border: `1px solid ${WARM.border}`, borderRadius: '10px', padding: '16px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
          ...
          <a href="/risk" style={{ fontSize: '11.5px', fontWeight: 700, color: WARM.orange, textDecoration: 'none' }}>View All →</a>
        </div>
        <div style={{ borderRadius: '8px', overflow: 'hidden', border: `1px solid ${WARM.border}` }}>
          <div style={{ display: 'grid', gridTemplateColumns: '70px 1.8fr 2.4fr 90px', padding: '8px 14px', background: '#f8fafc', fontSize: '10.5px', fontWeight: 700, color: WARM.muted, textTransform: 'uppercase', letterSpacing: '.4px' }}>
            <span>Type</span><span>Project</span><span>Alert</span><span>Time</span>
          </div>
          {alerts.map((a, i) => (
            <div key={i} style={{ display: 'grid', gridTemplateColumns: '70px 1.8fr 2.4fr 90px', padding: '10px 14px', background: i % 2 === 0 ? 'white' : '#fafafa', alignItems: 'center', borderTop: `1px solid ${WARM.border}` }}>
              <AlertTriangle size={16} color={a.type === 'danger' ? WARM.red : a.type === 'warning' ? WARM.amber : WARM.teal} />
